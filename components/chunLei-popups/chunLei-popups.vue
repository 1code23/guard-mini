<template>
	<!-- https://ext.dcloud.net.cn/plugin?id=801 -->
	<view class="mask" :class="!show?'':'mask-show'" :style="{backgroundColor:show?maskBg:'rgba(0,0,0,0)'}"
		...
		<view class="popups" :class="[theme]" :style="{top: popupsTop ,left: popupsLeft,flexDirection:direction}">
			...
			<view class="radioUserBox fc">
				<radio-group @change="radioChange">
					<label class="disfr ac " v-for="(item, index) in popData" :key="item.value">
						<view>
							<radio :value="item.id" :checked="index === current" style="transform:scale(0.4)" />
						</view>
						<view>{{item.nickName}}</view>
					</label>
				</radio-group>
				<label class="disfr ac ml10 addfacility" @click="addfacilityBtn">
					<view>
						<uni-icons type="plusempty" size="28rpx"></uni-icons>
					</view>
					<view class="ml10">添加设备</view>
				</label>
			</view>
			<slot></slot>
		</view>
...
<script>
	export default {
		...
		methods: {
			addfacilityBtn() {
				this.$emit('addfacilityBtn')
			},
			radioChange: function(evt) {
				for (let i = 0; i < this.popData.length; i++) {
					if (this.popData[i].id == evt.detail.value) {
						// this.bindChildreninfo = this.popData[i]
						// this.batterycapData(this.popData[i].device.batteryCap)
						// this.$store.commit('user/SET_BINDUSER', this.userList[i])
						this.current = i;
						this.$emit('tapPopup', this.popData[i])
						// this.ishsowUserList = false
						// this.earphoneList() //获取耳机详情数据
						// this.getapptheDayList(); //获取当天应用耗时列表
						// this.dataStatisticsList(this.$fun.YMD(),this.$fun.YMD())//获取app使用时长统计
						// this.appConfigInfoList() //获取应用信息
						break;
					}
				}
			},

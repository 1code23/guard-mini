<!-- 底部导航栏 https://ext.dcloud.net.cn/plugin?id=5593 -->
<!-- 
...
<template>
	<view class="ua__tabbar" :class="{'fixed': fixed}">
		<view class="ua__tabbar-wrap flexbox flex-alignc" :style="{'background': bgcolor}">
			<view v-for="(item, index) in tabs" :key="index" class="ua__tabbar-item flexbox flex-col"
				:class="currentTabIndex == index ? 'on' : ''" @click="switchTabs(index, item)">
				<view v-if="item.icon||item.img" class="ua__tabbar-icon" :class="{'dock': item.dock}">
					<template v-if="item.dock">
						<view class="dock-bg flexbox" :style="{'background': item.dockBg ? item.dockBg : activeColor}">
							<text v-if="item.icon" class="iconfont nvuefont" :class="item.icon"
								:style="{'color': (currentTabIndex == index && !item.dock ? activeColor : color), 'font-size': item.iconSize}">{{item.icon.charAt(1) == '' ? item.icon : ''}}</text>
							<image v-if="item.img" class="iconimg"
								:src="currentTabIndex == index && item.activeImg ? item.activeImg : item.img"
					...
					<view v-if="item.badge<=99&&item.badge>0" class="vui__badge ua__tabbar-icon__badge ua__tabbar-icon__badgeNomore dja">{{item.badge}}</text></view>
					<view v-if="item.badge>99" class="vui__badge ua__tabbar-icon__badge ua__tabbar-icon__badgeMore dja">99+</text></view>
					<view v-if="item.dot" class="vui__badge-dot ua__tabbar-icon__badgeDot"></view>
				</view>
				<view v-if="item.title&&!item.dock" class="ua__tabbar-title">
					<text class="ua__tabbar-title__text"
						:style="{'color': (currentTabIndex == index ? activeColor: color)}">{{item.title}}</text>
...
<script>
	export default {
		...
		data() {
			return {
				...
				tabs:[{
						...
						img: require('../../static/img/nohome2x.png'),
						activeImg: require('../../static/img/home2x.png'),
						title: `首页`,
					},
					{
						path: `/pages/place/place`,
						img: require('../../static/img/nolocation2x.png'),
						activeImg: require('../../static/img/location2x.png'),
						title: `位置`,
					},
					{
						path: `/pages/information/information`,
						img: require('../../static/img/notabbarmessage2x.png'),
						activeImg: require('../../static/img/tabbarMessage2x.png'),
						title: `消息`,
						badge: 0,
					},
					{
						path: `/pages/mine/mine`,
						img: require('../../static/img/nomes2x.png'),
						activeImg: require('../../static/img/mes2x.png'),
						title: `我的`,
					},
				]
			}
		...
		methods: {
			getMessNum(badge,remind,todo){
				this.tabs[2].badge=badge
				this.$emit('messageCounter',{remindNum:remind,todoNum:todo})
			},
			getReadNum(){
				this.$apiCom.messageCounter({isLoad:true}).then(res=>{
					let badge=0
					if(res.code==0){
					...
					this.tabs[2].badge=badge
					this.$emit('messageCounter',{remindNum:res.data.remind,todoNum:res.data.todo})
				})
			},
			// 匹配当前路由页面
			selectRoute(curPath) {
...
.ua__tabbar-item .vui__badge{
	...
	.ua__tabbar-item .dock .iconfont {
		color: #fff !important;
	}

	/* 字体图标/图片 */
	.ua__tabbar-item .iconfont {
		color: #9d9ea5;
		font-size: 45rpx;
		transition: color .3s;
	}
	...
	.ua__tabbar-title__text {
		color: #9d9ea5;
		/* font-size: 30rpx; */
		font-size: 20rpx;
	}

	.ua__tabbar-item.on .ua__tabbar-title__text {
		color: #f57b15;
	}
	...
	.ua__tabbar-icon__badgeNomore{
		width: 30rpx;
		height: 30rpx;
		right: -20rpx;
	}
    .ua__tabbar-icon__badgeMore{
		width: 50rpx;
		height: 33rpx;
		right: -35rpx;
	}

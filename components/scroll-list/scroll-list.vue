<template>
	<!-- https://ext.dcloud.net.cn/plugin?id=5303 -->
...
export default {
    ...
    data() {
        return {
            defaultOption: {
                ...
                refreshDelayed: 2000, // 刷新延迟
                refreshFinishDelayed: 2000, // 刷新完成后的延迟
                ...
                refreshSuccessText: new Date().toLocaleString(), // 刷新成功文字
    ...
    methods: {
        ...
        refreshSuccess() {
        	...
        	let time=this.$fun.getNowDate();
        	let nowTime=this.$fun.dateTimeWeekSplite(time)
        	// uni.setStorageSync('refreshTime', nowTime);
        	uni.setStorageSync('refreshTime', time);
        	this.refreshStateText = '最近刷新：'+nowTime;
        	//ios下拉后底部有空白
        	setTimeout(() => {
        		this.loadCompute()
    ...
    //             this.pulldowning = false;
				...
				// console.log(time);
				// uni.setStorageSync('refreshTime', 'hello');
                // 以下是解决ios下拉后底部有空白之前的代码
        ...
        refreshFail() {
			...
			// console.log(this.$fun.dateTimeWeekSplite(uni.getStorageSync('refreshTime')),11111,uni.getStorageSync('refreshTime'));
			let nowrefreshtime=uni.getStorageSync('refreshTime')?this.$fun.dateTimeWeekSplite(uni.getStorageSync('refreshTime')):'最近暂无刷新'
            this.refreshStateText =uni.getStorageSync('refreshTime')?'最近刷新时间：'+ nowrefreshtime:'最近暂无刷新';
            // this.refreshStateText =uni.getStorageSync('refreshTime')?'最近刷新时间：'+ uni.getStorageSync('refreshTime'):'最近暂无刷新';
            //ios下拉后底部有空白
			setTimeout(() => {
				this.loadCompute()
				this.pullDownHeight = 0;
